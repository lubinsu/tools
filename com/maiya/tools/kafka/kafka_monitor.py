...
def check_group_offsets(group_name, topics_monitor_o):
    ...
    for topic in topics:
        if topic in topics_monitor:
            ...
            try:
                partitions = zk.get_children('/consumers/%s/offsets/%s' % (group_name, topic))
                for partition in partitions:
                    log = '/consumers/%s/offsets/%s/%s' % (group_name, topic, partition)
                    if zk.exists(log):
                        data, stat = zk.get(log)
                        logsize = int(data)
                        lag = latest_offset[int(partition)].offset[0] - logsize
                        if lag >= 50:
...
if __name__ == '__main__':
    pro = Properties('resources/kafkaMonitor.properties').getProperties()
    java_consumer = {}
    spark_streaming = {}
    ...
    # if len(all_warn_lags) >= 1 and datetime.datetime.now().strftime('%M') == '00':
        for lag in all_warn_lags.items():
            ...
            if group_topic in java_consumer.keys():
                consumer_for_kill[java_consumer[group_topic]] = 'ssh -t -p 22 hadoop@%s /opt/cloudera/maiya/restartConsumer.sh' % java_consumer[group_topic]
            # 流式计算重启
            if group_topic in spark_streaming.keys():
                application_for_kill[get_application_id(spark_streaming, group_name, topic)] = 'yarn application -kill %s' % get_application_id(spark_streaming, group_name, topic)
